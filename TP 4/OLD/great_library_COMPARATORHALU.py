...
class Shelf:
    ...
    def search_buku(self, rak, buku):
        ...
        Apabila ditemukan, maka panggil get_book_description(), lalu kembalikan True sebagai status.
        """
        if buku in rak.get_buku():
            print("Buku ditemukan\n")
            rak.get_buku()[buku].get_book_description()
            return True

    # Setter
    def set_buku(self, nama, isi):
        # Inisiasi buku ke dalam dictionary, menggunakan judul buku sebagai key
        self.__kumpulan_buku[nama] = isi
    ...
    def list_buku(self, rak):
        ...
        for nama_rak in rak:
            print(nama_rak)
            nama_buku = rak[nama_rak].get_buku().keys()
            # Sort secara leksikografis
            nama_buku = sorted(sorted(nama_buku), key=str.upper)
...
class Library:
    ...
    def search_buku(self, nama):
        ...
        for rak in self.get_rak():
            status = Shelf.search_buku(Shelf, self.get_rak()[rak], nama)
            if status:
                return 0
    ...
    def add_buku(self, rak, daftar_perintah):
        ...
        if self.search_rak(rak):
            ...
            if success:
                ...
                print()
                self.get_rak()[rak].get_buku()[nama_buku].get_book_description()
